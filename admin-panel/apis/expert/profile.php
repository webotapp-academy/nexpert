        ...
        if ($exists) {
            $stmt = $pdo->prepare("
                UPDATE expert_profiles SET
                    ...
                    expertise_verticals = ?, category = ?, credentials = ?, experience_years = ?,
            ...
            // Log trust event
            TrustHelper::logEvent($pdo, 'expert_profile_updated', $user_id, null, [
                'full_name' => $full_name,
                'tagline' => $tagline,
                'expertise_verticals' => json_decode($expertise_verticals, true)
            ]);
            ...
            $stmt = $pdo->prepare("
                INSERT INTO expert_profiles (user_id, full_name, tagline, bio_short, bio_full,
                    expertise_verticals, category, credentials, experience_years, timezone, profile_photo)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            ");
            $stmt->execute([
                $user_id,
