...
ini_set('error_log', $_SERVER['DOCUMENT_ROOT'] . '/nexpert/error.log');

// Define BASE_PATH
$BASE_PATH = rtrim(str_replace('\\', '/', dirname($_SERVER['SCRIPT_NAME'])), '/');
$BASE_PATH = $BASE_PATH ? $BASE_PATH : '/';

// Ensure session configuration is loaded with full path
$session_config_path = $_SERVER['DOCUMENT_ROOT'] . '/nexpert/includes/session-config.php';
if (!file_exists($session_config_path)) {
    error_log("Session config file not found: " . $session_config_path);
...
// Ensure header and navigation files exist
$header_path = $_SERVER['DOCUMENT_ROOT'] . '/nexpert/includes/header.php';
$navigation_path = $_SERVER['DOCUMENT_ROOT'] . '/nexpert/includes/navigation.php';

if (!file_exists($header_path)) {
    error_log("Header file not found: " . $header_path);
    die("Header file missing");
}
if (!file_exists($navigation_path)) {
    error_log("Navigation file not found: " . $navigation_path);
    die("Navigation file missing");
}
