...
function logAndDisplayError($message) {
    error_log($message);
    echo "<div style='background-color: red; color: white; padding: 10px; margin: 10px 0;'>ERROR: $message</div>";
}
...
$expert_query = "
    SELECT 
        ...
        MIN(pr.amount) as min_hourly_rate
    FROM users u
...
if (!$conn) {
    logAndDisplayError("Database connection failed: " . mysqli_connect_error());
    exit;
}

// Execute query
$result = $conn->query($expert_query);

// Check query execution
if (!$result) {
    logAndDisplayError("Query failed: " . $conn->error);
    logAndDisplayError("Full Query: " . $expert_query);
    exit;
}

// Check if any results were found
$expert_count = $result ? $result->num_rows : 0;
if ($expert_count == 0) {
    logAndDisplayError("No experts found. Detailed debugging:");
    
    // Additional debugging queries
    $user_count_query = "SELECT COUNT(*) as total_users FROM users WHERE role = 'expert'";
    $user_count_result = $conn->query($user_count_query);
    $user_count = $user_count_result->fetch_assoc()['total_users'];
    logAndDisplayError("Total expert users: $user_count");

    $profile_query = "SELECT * FROM expert_profiles LIMIT 5";
    $profile_result = $conn->query($profile_query);
    while ($profile = $profile_result->fetch_assoc()) {
        logAndDisplayError("Expert Profile: " . print_r($profile, true));
    }
}
...
?>
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 sm:py-8">
        ...
        <div class="hidden md:flex justify-between items-center mb-6">
            <span id="result-count" class="text-sm sm:text-base text-gray-600">
                <?php 
                echo $expert_count . " Expert" . ($expert_count != 1 ? "s" : "") . " Found";
                ?>
            </span>
        ...
        <div class="md:hidden mb-4">
            <span id="result-count-mobile" class="text-sm text-gray-600">
                <?php 
                echo $expert_count . " Expert" . ($expert_count != 1 ? "s" : "") . " Found";
                ?>
            </span>
        ...
        <div>
                ...
                <div id="experts-grid" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-2 xl:grid-cols-3 gap-4 sm:gap-6">
                    <?php if ($result && $result->num_rows > 0): ?>
                        <?php while ($expert = $result->fetch_assoc()): ?>
                                ...
                                <div class="p-4">
                                    <div class="flex items-center mb-4">
                                        ...
                                        <div class="w-16 h-16 rounded-full overflow-hidden mr-4 flex-shrink-0">
                                            <img 
                                                src="<?= $expert['profile_photo'] ?? '/assets/default-avatar.png' ?>" 
                                        ...
                                        <!-- Name and Tagline -->
                                        <div>
                                            <h3 class="text-lg font-semibold text-gray-900"><?= htmlspecialchars($expert['full_name']) ?></h3>
                                            <p class="text-sm text-gray-600"><?= htmlspecialchars($expert['tagline']) ?></p>
                                        </div>
                                    </div>
                                    
                                    <!-- Bio -->
                                    <p class="text-sm text-gray-700 mb-4 line-clamp-3">
                                        <?= htmlspecialchars($expert['bio_short']) ?>
                                    </p>
                                    
                                    <!-- Expert Details -->
                                    <div class="grid grid-cols-3 gap-2 text-center mb-4">
                                        <div>
                                            <span class="block text-sm font-medium text-gray-600">Experience</span>
                                            <span class="text-primary font-bold"><?= $expert['experience_years'] ?> yrs</span>
                                        </div>
                                        <div>
                                            <span class="block text-sm font-medium text-gray-600">Rating</span>
                                            <span class="text-primary font-bold">
                                                <?= number_format($expert['rating_average'], 1) ?> 
                                                <span class="text-xs text-gray-500">(<?= $expert['total_reviews'] ?>)</span>
                                            </span>
                                        </div>
                                        <div>
                                            <span class="block text-sm font-medium text-gray-600">Rate</span>
                                            <span class="text-primary font-bold">₹<?= number_format($expert['min_hourly_rate']) ?>/hr</span>
                                        </div>
                                    </div>
                                    ...
                                    <div class="flex flex-wrap gap-2 mb-4">
                                        <?php 
                                        $categories = explode(',', $expert['categories']);
                                        foreach ($categories as $category): ?>
                                            <span class="px-2 py-1 bg-gray-100 text-xs text-gray-700 rounded-full">
                                    ...
                                    <div class="flex gap-2">
                                        <a href="?panel=learner&page=expert-profile&id=<?= $expert['id'] ?>" 
                                           class="flex-1 text-center px-4 py-2 bg-primary text-white rounded-lg hover:bg-secondary transition">
                                            View Profile
                                        </a>
                                        <a href="?panel=learner&page=booking&expert_id=<?= $expert['id'] ?>" 
                                           class="flex-1 text-center px-4 py-2 border-2 border-primary text-primary rounded-lg hover:bg-primary hover:text-white transition">
                                            Book Session
                                        </a>
                                    </div>
                                </div>
                    ...
                    <?php else: ?>
                        <div class="col-span-full text-center py-8">
                            <p class="text-gray-600 text-lg">No experts found at the moment.</p>
                            <p class="text-gray-500 text-sm mt-2">Please check back later or adjust your filters.</p>
                        </div>
                    <?php endif; ?>
                </div>
